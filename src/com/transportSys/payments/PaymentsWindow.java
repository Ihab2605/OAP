...
public class PaymentsWindow {
	...
	private void initialize() {
		...
		table = new JTable() {
			...
			public String getToolTipText(MouseEvent e) {
				...
				if (row != -1 && col != -1) {
					...
					if (value != null) {
						return wrapTooltipText(value.toString(), 200);
					}
				}
		...
		KeyAdapter searchAdapter = new KeyAdapter() {
			...
			public void keyReleased(KeyEvent arg0) {
				searchPayments(textAreaSearchCustomerNumber.getText(), textAreaSearchPaymentDate.getText());
			}
		};
		...
		/*
		 * // Commenting this part out instead of delete incase its necessary to include
		 * these buttons.
		 * 
		 * AddAction addButton = new AddAction("Add", table, "customerNumber",
		 * "payments"); addButton.setFont(new Font("Yu Gothic UI Semibold", Font.PLAIN,
		 * 12)); addButton.setText("ADD"); addButton.setMaximumSize(buttonSize);
		 * panelButtons.add(addButton);
		 * 
		 * panelButtons.add(Box.createVerticalStrut(10));
		 * 
		 * EditAction editButton = new EditAction("Edit", table, "customerNumber",
		 * "payments"); editButton.setFont(new Font("Yu Gothic UI Semibold", Font.PLAIN,
		 * 12)); editButton.setText("EDIT"); editButton.setMaximumSize(buttonSize);
		 * panelButtons.add(editButton);
		 * 
		 * panelButtons.add(Box.createVerticalStrut(10));
		 * 
		 * DeleteAction deleteButton = new DeleteAction("Delete", table,
		 ...
		 * deleteButton.setText("DELETE"); deleteButton.setMaximumSize(buttonSize);
		 * panelButtons.add(deleteButton);
		 * 
		 * panelButtons.revalidate(); panelButtons.repaint();
		 */
	...
	private void searchPayments(String customerNumberSearchText, String paymentDateSearchText) {
		try {
